  ...
  <v-row no-gutters>
    <v-col cols="12" md="12">
      <v-toolbar dark color="primary pl-2 pr-2" density="compact">
        <v-toolbar-title>Language Photo</v-toolbar-title>
        <v-spacer></v-spacer>
...
export default {
  ...
  mounted: function() {
    this.file = `${axios.defaults.baseURL}/languagephoto/${this.language?.lanPhoto}.png`;
  },
  methods: {
...
var formData = new FormData();
      ...
      languageService
        .updateLanPhoto(formData,this.language.languagesId)
        .then(response => {
